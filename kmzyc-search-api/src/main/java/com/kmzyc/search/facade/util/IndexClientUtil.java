...
public class IndexClientUtil {
     ...
     * @return
     */
    public static String getAnalyzeTerms(String queryWord) {
        try {
            String analyzeURL = Configuration.getContextProperty("es.analyzeURL");
            return HttpClientUtil.URLGet(analyzeURL + URLEncoder.encode(queryWord, "utf-8"));
        } catch (UnsupportedEncodingException e) {
            logger.error("URLEncoder encode exception !", e);
        }
        return null;
    }
     ...
     * @since 20160511
     */
    public void addIndex(String indexName, String indexType, String docId, JSONObject doc) {
        ...
        IndexRequestBuilder requestBuilder =
                transportClient.prepareIndex(indexName, indexType, docId).setRefresh(true);
        // 新增索引
        ...
        for (Map.Entry<String, JSONObject> entry : docs.entrySet()) {
            ...
            if (null != doc) {
                IndexRequestBuilder indexRequestBuilder = transportClient
                        .prepareIndex(indexName, indexType, docId).setSource(doc.toString());
     ...
     * @since 20160511
     */
    public void deleteIndex(String indexName, String indexType, String docId) {
        ...
        DeleteRequestBuilder deleteRequestBuilder =
                transportClient.prepareDelete(indexName, indexType, docId).setRefresh(true);
        // 删除索引
